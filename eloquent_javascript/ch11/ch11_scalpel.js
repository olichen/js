...
// code above from chapter

async function locateScalpel(nest) {
    let loc = nest.name;
    while (true) {
        let next = await anyStorage(nest, loc, "scalpel");
        if (next == loc) return loc;
        loc = next;
    }
}
...
function locateScalpel2(nest) {
    function searchNode(node) {
        return anyStorage(nest, node, "scalpel").then(next => {
            if (next == node) return node;
            else return searchNode(next);
        });
    }
    return searchNode(nest.name);
}
